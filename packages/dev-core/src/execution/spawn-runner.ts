 ...
 * the user explicitly opts into live execution with `--execute`; the default
 * `run` path uses {@link DryRunCommandRunner}. Never runs a shell — args are
 * passed as an array so there is no shell-injection surface (master prompt
 * §Security).
 */
